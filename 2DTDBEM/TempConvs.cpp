...
void CTempConvs::compute2(VECTOR& P, CLagrange_interp& intTB, CLagrange_interp& TB, CLagrange_interp& dTB,
	VECTOR& Fh, VECTOR& Fs, VECTOR& dF)
{
	UINT num_partitions = intTB.m_partition.size() - 1;
	UINT num_degree = intTB.m_coeffs.n_cols - 1;
...
#pragma omp parallel for schedule(static)
	for (int ii = 0; ii < P.size(); ii++)
	{
		...
		for (int i = 0; i<(int)num_partitions; i++)
		{
			...
			Ia(ii,0) = log(tmp2);
			Ib(ii, 0) = log(tmp4);
			dIa(ii, 0) = (da + dI1a) / tmp2;
			dIb(ii, 0) = (db + dI1b) / tmp4;

			//Convolve polynomial coefficients with solved integral
			tmp1 = Ib(ii, 0) - Ia(ii, 0);
			Fh(ii) += tmp1 * intTB.m_coeffs(i, num_degree);
			Fs(ii) += tmp1 * TB.m_coeffs(i, num_degree);
			dF(ii) += (dIb(ii, 0) - dIa(ii, 0)) * dTB.m_coeffs(i, num_degree);

			//Second integral equation, I_1
			if (num_degree > 0)
			{
				//I_1
				Ia(ii, 1) = sqrt(a*a - PP*PP);
				Ib(ii, 1) = sqrt(b*b - PP*PP);
				...
				//Convolve
				tmp1 = Ib(ii, 1) - Ia(ii, 1);
				Fh(ii) += tmp1 * intTB.m_coeffs(i, num_degree - 1);
				Fs(ii) += tmp1 * TB.m_coeffs(i, num_degree - 1);
				dF(ii) += (dIb(ii, 1) - dIa(ii, 1)) * dTB.m_coeffs(i, num_degree - 1);
				...
				for (int n = 2; n <= (int)num_degree; n++)
				{
					...
					// I_2...I_p
					Ia(ii, n) = (dotpow(a, n - 1) * Ia(ii, 1) + (n - 1) * PP*PP * Ia(ii, n-2)) * over_n;
					Ib(ii, n) = (dotpow(b, n - 1) * Ib(ii, 1) + (n - 1) * PP*PP * Ib(ii, n-2)) * over_n;

					//dI_2 ... dI_p
					tmp1 = dotpow(da, n - 1) * Ia(ii, 1);
					tmp2 = dotpow(a, n - 1) * dIa(ii, 1);
					tmp3 = (n - 1)*(2 * PP * Ia(ii, n-2) + PP*PP * dIa(ii, n-2));
					dIa(ii, n) = (tmp1 + tmp2 + tmp3) * over_n;

					tmp1 = dotpow(db, n - 1) * Ib(ii, 1);
					tmp2 = dotpow(b, n - 1) * dIb(ii, 1);
					tmp3 = (n - 1)*(2 * PP * Ib(ii, n-2) + PP*PP * dIb(ii, n-2));
					dIb(ii, n) = (tmp1 + tmp2 + tmp3) * over_n;

					// Convolve
					tmp1 = Ib(ii, n) - Ia(ii, n);
					Fh(ii) += tmp1 * intTB.m_coeffs(i, num_degree - n);
					Fs(ii) += tmp1 * TB.m_coeffs(i, num_degree - n);
					dF(ii) += (dIb(ii, n) - dIa(ii, n)) * dTB.m_coeffs(i, num_degree - n);
